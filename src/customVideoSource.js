...
async function join() {
  ...
  if (currentStream == "camera") {
    ...
    $("#local-player-name").text(`localVideo(${options.uid})`);
    console.log("publish success");
  } else if (currentStream == "funk2") {
    var stream = document.getElementById("canid2").captureStream(30);
    [options.uid, localTracks.videoTrack] = await Promise.all([
      // Join the channel.
      client.join(options.appid, options.channel, options.token || null, options.uid || null),
      // Create tracks to the customized video source.
    ...
    $("#local-player-name").text(`localVideo(${options.uid})`);
    console.log("publish success");
  } else if (currentStream == "funk3") {
    cany = document.createElement('canvas');
    var ctx = cany.getContext('2d');
    var img = new Image;
    img.onload = async function () {
      ctx.drawImage(img, 0, 0); // Or at whatever offset you like
      var stream = cany.captureStream(30);
      setInterval(() => {
        ctx.drawImage(cany, 0, 0)
      }, 50);
      [options.uid, localTracks.videoTrack] = await Promise.all([
        // Join the channel.
        client.join(options.appid, options.channel, options.token || null, options.uid || null),
    ...
    img.src = "https://sa-utils.agora.io/files/89201587990641.jpg"
  } else {
